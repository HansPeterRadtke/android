...
public final class ReliableUploader {
    ...
    private void reconcile(ReliableSessionManifest snapshot) throws Exception {
        ...
        ReliableUploadClient.Status status = client.status(snapshot);
        ...
        for (ReliableSessionManifest.Segment segment : manifest.orderedSegments()) {
            ...
            if (remote != null) {
                ...
                store.markRemoteAccepted(sessionId, segment.seq, status.serverId,
    ...
    private static String humanFailure(Throwable failure) {
        ...
        String message = root == null || root.getMessage() == null ? "no exception message"
                : root.getMessage();
        return "Chunk synchronization failed: " + type + ": " + message
                + ". Every unconfirmed chunk remains on the phone and will retry.";
    }
}
