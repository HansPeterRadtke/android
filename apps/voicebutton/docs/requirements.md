...
## Version 0.15 continuous foreground recording and interruption alarm

Once Start succeeds, closing the activity, pressing Back, pressing Home, or swiping the task away SHALL NOT stop capture or synchronization. The microphone foreground service, CPU wake lock, two-second chunk writer and resumable uploader SHALL continue until the user presses Pause or Finish. Pending audio and transcript work SHALL keep the service foreground and sticky until every required acknowledgement is complete.

Any unexpected capture-stopping failure SHALL start an alarm-stream tone immediately and repeat it approximately every four seconds. A high-priority recording-error notification and a visible in-app Silence alarm control SHALL be provided. Silencing stops only the sound; automatic microphone recovery continues with bounded exponential retry. Successful AudioRecord restart clears the incident automatically. Pause automatic recovery SHALL mark the session paused and stop both retry and alarm. Finish SHALL close the session permanently.
