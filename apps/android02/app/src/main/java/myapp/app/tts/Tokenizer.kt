...
object Tokenizer {
    // Kokoro v1.0: model input is padded with 0 at start/end, so the *unpadded* token
    // length should stay within 510 to keep total sequence length <= 512.
...
"""
    ...
    fun tokenize(phonemes: String): LongArray {
        ...
        // Match kokoro-onnx python behavior: no pads here; caller pads with 0 at start/end.
        val out = LongArray(minOf(phonemes.length, MAX_TOKENS))
        var count = 0
        for (ch in phonemes) {
