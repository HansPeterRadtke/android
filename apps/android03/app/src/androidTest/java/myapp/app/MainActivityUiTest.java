...
public class MainActivityUiTest {
  ...
  @Test public void defaultOverviewShowsTrustConversationCurrentMessageAndPrimaryActions() {
    onView(withId(R.id.voice_status)).check(matches(isDisplayed()));
    onView(withId(R.id.voice_status_detail)).check(matches(isDisplayed()));
    onView(withId(R.id.voice_component_health)).check(matches(isDisplayed()));
    onView(withId(R.id.voice_conversation)).check(matches(isDisplayed()));
    onView(withId(R.id.voice_transcript_panel)).check(matches(isDisplayed()));
    ...
    onView(withId(R.id.voice_diagnostics)).check(matches(isDisplayed())).check(matches(isClickable()));
    onView(withId(R.id.voice_worker_status)).check(matches(withEffectiveVisibility(GONE)));
    onView(withId(R.id.voice_replay_row)).check(matches(withEffectiveVisibility(GONE)));
  ...
  @Test public void settingsExposeVersionAndAutomationContinuum() {
    ...
    onView(withText(containsString("Voice Agent 1.3.0"))).check(matches(isDisplayed()));
    ...
    pressBack();
    onView(withId(R.id.voice_diagnostics)).perform(click());
